...
/*
|--------------------------------------------------------------------------
| ÄÄƒng kĂ½ báº±ng facebook , google, ect...
|--------------------------------------------------------------------------
*/
...
function ChangeDeliveryPrice(){
    ...
    $('.delivery_option').each(function(){
        ...
        $(this).find('.delivery_option_price').text(full_price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".")+'Ä‘');
    });
}
...
CIRCLECLOUD.FormLogin = function () {
...
CIRCLECLOUD.OwlProDetail = function () {
...
CIRCLECLOUD.Cart = function () {
	...
	$("body").on("click", ".del-procart", function () {
		...
		Swal.fire({
			title: '<h5>Báº¡n cĂ³ cháº¯c muá»‘n xĂ³a sáº£n pháº©m nĂ y?</h5>',
			showDenyButton: true,
			confirmButtonText: 'Äá»“ng Ă½',
			denyButtonText: 'Há»§y bá»',
		}).then((result) => {
					...
					if (result.success == false) {
						if (result.data) {
							...
							for (let i in data) {
								...
								if (data[i] > 0) {
									$('.cart-warning-' + i).text('(Sáº£n pháº©m nĂ y chá»‰ cĂ²n ' + data[i] + ' sáº£n pháº©m)');
									//console.log(data[i]);
								} else {
									$('.cart-warning-' + i).text('(Sáº£n pháº©m nĂ y vá»«a má»›i háº¿t hĂ ng)');
									//console.log(0);		
								}
							}
						}
...
$(document).ready(function () {
	...
	CIRCLECLOUD.AllPage();
	CIRCLECLOUD.OwlPage();
	CIRCLECLOUD.OwlProDetail();
	CIRCLECLOUD.Cart();
	CIRCLECLOUD.AOS();
	CIRCLECLOUD.FormLogin();
});
...
function ColorClick(e) {
	...
	var _token = $('input[name="_token"]').val(); // token Ä‘á»ƒ mĂ£ hĂ³a dá»¯ liá»‡u
